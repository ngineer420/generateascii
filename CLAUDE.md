...
The image tool cannot share its settings, because the person who opens a link
does not have the image. Its "Copy link" button therefore puts the art itself in
the URL fragment: `/image-to-ascii#a=<payload>`. The payload is JSON with `v`,
`c` (columns), `t` (the rows joined by `\n`), `m` (`mono` or `color`), `b`
(`dark` or `light`), and either `k` (the mono colour) or `p` (two base64url
characters of 12-bit colour for each inked cell). `CompressionStream("deflate-raw")`
packs it, and base64url encodes it. A browser never sends the fragment to a
server. A link wins over the image restored from sessionStorage, and the button
refuses a link longer than 64,000 characters. `/s/` only tells a visitor that
the old stored share links stopped working. Nothing is stored anywhere.
